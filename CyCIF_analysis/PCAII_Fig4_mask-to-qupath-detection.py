...
df_feature = pd.read_csv(
    r"\\research.files.med.harvard.edu\HITS\lsp-analysis\cycif-production\16-Pre-Cancer-Atlas-for-Melanoma\PCA_p16_e41_tumor_intrinsic\quantified\LSP11349\segmentation\unmicst-LSP11349\LSP11349_classical_neighborhood_entropy_df_segmask_plots_Fig3.csv"
)
idxr = np.full(df_feature["CellID"].max() + 1, np.nan, dtype="float32")
idxr[df_feature["CellID"]] = df_feature["entropy"]
        ...
        for idx, row in tqdm.tqdm(df.iterrows(), total=len(df)):
            ...
            row_dict = {
                ...
                **nucl_geom,
                "properties": {
                    "objectType": "cell",
                    "measurements": {"label": idx, "entropy": float(idxr[idx])},
                },
            }
            line_break = "\n" if idx == last else ",\n"
